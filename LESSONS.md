...
## 2026-08-13: the test fixture is a striped field, which cannot be tracked at all
Symptom: the new motion-tracking e2e failed at every point tried — 0.35,0.5 then 0.5,0.3 then 0.2,0.7 then 0.7,0.6 — with "could not track". It looked like the tracker was broken, after it had passed 49 unit tests.
Root cause: `motion-1280x720-5s.mp4` is a field of diagonal stripes with one small orange circle. A stripe has a strong gradient across it and almost none along it, so the smaller eigenvalue of the structure tensor is ~0 and the point is correctly reported lost — the aperture problem, working as designed. Every point I guessed was stripe. Screenshotting the picker showed the circle at 0.09,0.5; picking that tracked all 150 frames.
Fix: pick the circle, and add a test that picking a stripe *is* refused — the honest-failure path is now covered rather than being the accidental subject of every test. Also corrected the message: "could not read enough frames" sent the user hunting for a decode fault, when the real answer is "that region has nothing to lock onto".
Follow-up (same day): writing the lesson down did not stop it recurring, so the fix moved into the code. `boxInView` in `apps/web/e2e/helpers.ts` scrolls before measuring and throws if the element has no box, and the existing drag call sites in `curves.spec.ts` and `markers.spec.ts` now go through it. A lesson that relies on being remembered will be forgotten; make the safe thing the easy thing.

Why: two habits. **Look at the fixture** — one screenshot of the picking canvas would have saved four failed runs and 20 minutes. And a correct refusal is indistinguishable from a broken feature if you only read the pass/fail; the message has to name which it is, or every future failure here starts by suspecting the wrong thing.

## 2026-08-13: the mutation that passed, and the comment that was wrong
Symptom: a mutation removing the demosaic's neighbour-search widening (5×5 back to 3×3) passed all 75 tests. The code comment justifying the widening said a 3×3 "does not always contain a site of every colour, for red at a blue site on some patterns".
Root cause: the comment was simply false. For a 2×2 Bayer sensor every phase has all three colours within one step, so the widening is never reached — and no test covered a pattern where it would be. The comment had been written from plausible reasoning rather than from working the case out.
Fix: correct the comment to say what is true (the widening is for sparser, non-Bayer repeats, which this supports because it reads the CFA pattern from the file), and add a test with a 4×4 repeat where the 3×3 genuinely comes up empty. The mutation now fails.
Why: a surviving mutation is not always a missing test — sometimes it is a wrong belief, and the missing test is downstream of that. When a mutation passes, check whether the code is unnecessary before writing a test to protect it, or you enshrine the mistake. Note also which direction the error ran: the comment overstated the need, so the code was harmlessly over-general; had it understated, the same confident-sounding reasoning would have removed a branch that was load-bearing.

## 2026-08-13: "installed" printed, nothing installed — and it reached main
Symptom: `USER_MANUAL.md` claimed camera raw support and the Word manual said nothing about it. The raw-import commit's message said it updated the docx; `git log -- docs/*.docx` showed the file's last change was two commits earlier.
Root cause: the docx was patched by an ad-hoc `unzip && python-heredoc && zip && cp` chain. The Python's anchor no longer matched and it raised — but the following `&&` stages ran anyway, because the heredoc's failure did not stop the chain as written, so the rezip ran on unmodified content and copied a byte-identical file back while printing "installed". `manual:check` did not catch it: it verifies the docx *changed*, not that it says anything in particular, and the file had been changed by an earlier hunk in the same commit.
Fix: `scripts/patch-manual-docx.mjs`. It patches and then **reads the written file back** to confirm each phrase is present, exiting non-zero if not, if the anchor is missing, if the anchor is ambiguous, or if the result is not well-formed XML. Its own failure modes were exercised before use.
Follow-up: the first fix was only half of it. The script wrote the Word manual but the Markdown was still edited ad-hoc, and the same silent-no-match failure then happened there — twice the two formats disagreed, and once *neither* had the text while the commit claimed both did. So my earlier diagnosis was also wrong: I reported "the Markdown claims a feature the Word manual never mentioned" when in fact the Markdown never claimed it either. The script now writes both, and it checks both anchors *before* writing either — its own first version patched the docx and then failed on the Markdown, leaving exactly the divergence it exists to prevent, which its own first test caught.

Why: this is the fourth time in one session that a pipe or `&&` swallowed a non-zero exit, and the first that shipped. Two rules follow. **Never branch on a command whose exit code has been through a pipe** — `cmd | tail -n` reports `tail`'s status, always zero. And for anything that edits a file, **verify by reading the file back**, not by trusting the writer's exit status; a gate that checks "did it change" is not a gate that checks "is it right".

## 2026-08-13: a click that waits forever on an element with no box
Symptom: every test in a new spec failed at the 3-minute test timeout, including one that only checked two buttons were visible. It looked like the app was broken or hung.
Root cause: `selectClip` switched to **photo** mode and then clicked `.clip`. Photo mode hides the timeline, so the clip is in the DOM with `boundingBox()` returning null — Playwright's actionability check then waits for it to become clickable, which it never does, and the whole test burns its timeout before the first assertion runs. The app was fine: a debug run imported media and rendered a clip with no errors. Every other spec that clicks a clip uses video mode, which is why none of them hit this.
Fix: use video mode. The grade path is identical in both, so nothing is lost.
Why: three habits. **A timeout is not evidence of a hang** — read which line it timed out on, because "waiting for locator" and "assertion failed" are different problems. **`locator resolved to <div…>` in a Playwright error means the element exists and is not actionable**, which points at visibility or stability, not at selectors. And when a whole spec fails identically, suspect the shared setup helper before the feature: the one test that should have been trivial failing too was the clue, and I looked at the feature first anyway.
